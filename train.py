...
def train(data_loader, model, criterion, optimizer, epoch):
    ...
    for batch_idx, data in enumerate(data_loader):
        ...
        if batch_idx % log_step == 0:
            print('Train Epoch: {} [{}/{}] \t Loss: {:.4f}'.format(epoch, batch_idx, len(data_loader),
                                                                   total_loss / log_step))
            total_loss = 0
    print("****************")
        ...
        for batch_idx, data in enumerate(data_loader):
            ...
            if demo:
                for idx, img, prediction in zip(range(args.batch_size), imgs, predictions):
                    ...
                    cv2.imwrite(os.path.join(weighted_img_target_dir, str(batch_idx) + "_" + str(idx) + ".jpg"),
                                weighted_img)
                    cv2.imwrite(os.path.join(pred_target_dir, str(batch_idx) + "_" + str(idx) + "_pred.png"),
                                prediction)
